<div align="center">
  
# MantaTools

<img src="https://raw.githubusercontent.com/MantaTools/MantaTools/main/resources/icon.png" alt="MantaTools Logo" width="150" height="150">

### Your Ultimate Game Management Companion

[![Latest Release](https://img.shields.io/github/v/release/MantaTools/MantaTools-new?style=for-the-badge&logo=github&logoColor=white&labelColor=1a1a2e&color=4361ee)](https://github.com/MantaTools/MantaTools/releases)
[![Downloads](https://img.shields.io/github/downloads/MantaTools/MantaTools-new/total?style=for-the-badge&logo=download&logoColor=white&labelColor=1a1a2e&color=7209b7)](https://github.com/MantaTools/MantaTools/releases)
[![Windows](https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white)](https://github.com/MantaTools/MantaTools/releases)

<br>

[**⬇️ Download Latest**](https://github.com/MantaTools/MantaTools/releases/latest) • [**📋 Changelog**](https://github.com/MantaTools/MantaTools/releases) • [**💬 Discord**](https://discord.gg/mantagames)

</div>

---

## ✨ Features

<table>
<tr>
<td width="50%">

### 🎮 Game Catalogue
Browse through an extensive library with Premium & Standard game badges

### 🔓 Denuvo Activation  
Easy one-click activation for Denuvo-protected games

### 🔧 Game Bypass
Launcher bypass for Activision, EA, Rockstar, and Ubisoft titles

</td>
<td width="50%">

### 🌐 Multiplayer Fixes
Download and install online/multiplayer fixes seamlessly

### 📦 DLC Unlocker
Unlock DLCs with integrated SmokeAPI support

### 🎨 Modern UI
Sleek dark theme with smooth animations

</td>
</tr>
</table>

---

## 📥 Download & Installation

### System Requirements
- **OS:** Windows 10/11 (64-bit)
- **RAM:** 4 GB minimum
- **Storage:** 200 MB

### Installation Steps

1. **Download** the latest release from [Releases](https://github.com/MantaTools/MantaTools-new/releases/latest)
2. **Choose your version:**
   - `mantatools-x.x.x-setup.exe` — Standard installer
   - `mantatools-x.x.x-portable.exe` — Portable version (no installation required)
3. **Run** the application and enjoy!

> ⚠️ **Note:** Windows SmartScreen may show a warning. Click "More info" → "Run anyway" to proceed.

---

## 🖼️ Screenshots

<div align="center">

| Home | Catalogue | Game Details |
|:---:|:---:|:---:|
| ![Home](https://via.placeholder.com/300x180/1a1a2e/ffffff?text=Home) | ![Catalogue](https://via.placeholder.com/300x180/1a1a2e/ffffff?text=Catalogue) | ![Details](https://via.placeholder.com/300x180/1a1a2e/ffffff?text=Details) |

</div>

---

## 🔄 Auto Updates

MantaTools automatically checks for updates on startup. When a new version is available, you'll be prompted to download and install it seamlessly.

---

## 🤝 Support

<div align="center">

| Platform | Link |
|:---:|:---:|
| 💬 Discord | [Join Server](https://discord.gg/mantagames) |
| 🌐 Website | [mantagames.store](https://mantagames.store) |
| 📧 Email | support@mantagames.store |

</div>

---

## 📝 Changelog

See the [Releases](https://github.com/MantaTools/MantaTools-new/releases) page for detailed version history and changelogs.

---

<div align="center">

### ⭐ Star this repository if you find it useful!

<br>

Made with ❤️ by **MantaTools Team**

<sub>© 2025 MantaTools. All rights reserved.</sub>

</div>
